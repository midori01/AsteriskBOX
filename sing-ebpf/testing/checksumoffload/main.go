...
func loadConfig() (config, error) {
	...
	for name, value := range map[string]string{
		...
		"FORCE_INTERCEPT_PREFIX":        os.Getenv("FORCE_INTERCEPT_PREFIX"),
		"REMOTE_FORCE_INTERCEPT_TARGET": c.forceInterceptTarget,
	} {
		if value == "" {
			return c, fmt.Errorf("%s is required", name)
		}
	}
	forceInterceptPrefix, err := netip.ParsePrefix(os.Getenv("FORCE_INTERCEPT_PREFIX"))
	if err != nil {
		return c, fmt.Errorf("FORCE_INTERCEPT_PREFIX: %w", err)
	}
	forceInterceptTarget, err := netip.ParseAddr(c.forceInterceptTarget)
	if err != nil {
		return c, fmt.Errorf("REMOTE_FORCE_INTERCEPT_TARGET: %w", err)
	}
	if !forceInterceptPrefix.Contains(forceInterceptTarget) {
		return c, fmt.Errorf("REMOTE_FORCE_INTERCEPT_TARGET is outside FORCE_INTERCEPT_PREFIX")
	}
	if c.remoteIPv6 != "" {
...
func (r *runner) runCombination(ctx context.Context, combination offloadCombination) {
	...
	if r.config.testRole == "local" || r.config.testRole == "both" {
		r.checkPing(ctx, combination.name, false, 4, r.config.forceInterceptTarget)
	...
	if r.config.testRole == "shared" || r.config.testRole == "both" {
		r.checkPing(ctx, combination.name, true, 4, r.config.forceInterceptTarget)
		if r.config.remoteIPv6 != "" {
...
func (r *runner) checkPing(ctx context.Context, label string, downstream bool, family int, target string) {
	check := fmt.Sprintf("local_icmp_echo_reply_v%d", family)
	if downstream {
...
func (r *runner) checkTCP(ctx context.Context, label string, downstream bool) {
	...
	if downstream {
		...
		if _, err = r.remote(ctx, true, bytes.NewReader(payload), "cat > "+sendPath); err == nil {
			_, err = r.remote(ctx, true, nil, "timeout 25 nc -q1 "+r.config.forceInterceptTarget+" "+strconv.Itoa(r.config.tcpPort)+" < "+sendPath)
		}
	} else {
		transferCtx, cancel := context.WithTimeout(ctx, 25*time.Second)
		_, err = commandOutput(transferCtx, bytes.NewReader(payload), "nc", "-q1", r.config.forceInterceptTarget, strconv.Itoa(r.config.tcpPort))
		cancel()
	}
...
func (r *runner) checkUDP(ctx context.Context, label string, downstream bool) {
	...
	for attempt := 1; attempt <= 3; attempt++ {
		...
		if downstream {
			...
			if _, err = r.remote(ctx, true, bytes.NewReader(payload), "cat > "+sendPath); err == nil {
				_, err = r.remote(ctx, true, nil, "nc -u -q1 -w2 "+r.config.forceInterceptTarget+" "+strconv.Itoa(r.config.udpPort)+" < "+sendPath)
			}
		} else {
			_, err = commandOutput(ctx, bytes.NewReader(payload), "nc", "-u", "-q1", "-w2", r.config.forceInterceptTarget, strconv.Itoa(r.config.udpPort))
		}
		waitCommand(listener)
