...
// role, including the optional ForceIntercept ICMP companion.
	...
	if a.attachmentType == "tcx" {
		if a.role.local {
			...
			if icmpEchoReplyEnabled {
				...
				if err != nil {
					return false, E.Cause(err, "inspect TCX local icmp_echo_reply attachment on interface ", a.interfaceName)
				}
				if !attached {
					return false, nil
				}
		...
		if a.role.shared {
			...
			if icmpEchoReplyEnabled {
				...
				if err != nil {
					return false, E.Cause(err, "inspect TCX shared icmp_echo_reply attachment on interface ", a.interfaceName)
				}
				if !attached {
					return false, nil
				}
		...
		if a.role.local {
			...
			if icmpEchoReplyEnabled {
				if err = clearStaleTCXLink(&a.localICMPLink, a.interfaceIndex, CiliumEBPF.AttachTCXEgress); err != nil {
					return E.Cause(err, "inspect TCX local icmp_echo_reply attachment on interface ", a.interfaceName)
				}
			}
		}
		if a.role.shared {
			...
			if icmpEchoReplyEnabled {
				if err = clearStaleTCXLink(&a.sharedICMPLink, a.interfaceIndex, CiliumEBPF.AttachTCXIngress); err != nil {
					return E.Cause(err, "inspect TCX shared icmp_echo_reply attachment on interface ", a.interfaceName)
				}
			}
		}
		return nil
