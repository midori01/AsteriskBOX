...
// TestICMPEchoReplyHealthCheckDetectsAndRepairsAMissingClsactFilter drives the
// real detection-and-repair path filtersAttached feeds: attach normally
// (both the ordinary local filter and the icmp_echo_reply filter land, and a
// real ping is answered), detach only the icmp_echo_reply filter behind the
...
func TestICMPEchoReplyHealthCheckDetectsAndRepairsAMissingClsactFilter(t *testing.T) {
	...
	if attachment.localICMPFilter == nil {
		t.Fatal("the icmp_echo_reply filter was not attached alongside the ordinary one")
	}
	...
	// Simulate the icmp_echo_reply filter vanishing on its own — an external
	...
	if err = detachTCFilter(attachment.localICMPFilter); err != nil {
		t.Fatalf("detach the icmp_echo_reply filter to simulate drift: %v", err)
	}
	...
	if attached {
		t.Fatal("filtersAttached reported this attachment healthy with the icmp_echo_reply filter missing")
	}

	if err = pingICMPEchoReplyTarget(t, forceInterceptTarget, 2*time.Second); err == nil {
		t.Fatal("ping still answered after the icmp_echo_reply filter was detached — the drift is not real")
	}
	...
	if err = dataPlane.reconcile("sbicmph0", nil, nil); err != nil {
		t.Fatalf("reconcile did not repair the missing icmp_echo_reply filter: %v", err)
	}
	...
	if repaired.localICMPFilter == nil {
		t.Fatal("repair did not restore the icmp_echo_reply filter")
	}
