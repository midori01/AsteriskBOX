...
// TestAttachTCInterfaceAddsTheICMPEchoReplyFilterWhenEnabled attaches to a real
// veth in a private network namespace with a real icmp_echo_reply-enabled
// backend, and checks by real netlink filter list — not by inspecting the Go
// struct — that both the ordinary local filter and the icmp_echo_reply local
...
func TestAttachTCInterfaceAddsTheICMPEchoReplyFilterWhenEnabled(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("inspect the icmp_echo_reply local reply filter: %v", err)
	}
	if !icmpAttached {
		t.Fatal("the icmp_echo_reply local reply filter was not attached alongside the ordinary one")
	}
	...
	if icmpAttached, err = tcFilterAttached(link, netlink.HANDLE_MIN_EGRESS, "sb_icmp_local", tcLocalICMPReplyFilterHandle, priority); err != nil {
		t.Fatalf("inspect the icmp_echo_reply local reply filter after close: %v", err)
	} else if icmpAttached {
		t.Fatal("the icmp_echo_reply local reply filter survived Close")
	}
...
// TestAttachTCInterfaceSkipsTheICMPEchoReplyFilterWhenDisabled is the control:
...
func TestAttachTCInterfaceSkipsTheICMPEchoReplyFilterWhenDisabled(t *testing.T) {
	...
	if icmpAttached, err := tcFilterAttached(link, netlink.HANDLE_MIN_EGRESS, "sb_icmp_local", tcLocalICMPReplyFilterHandle, priority); err != nil {
		...
		t.Fatal("the icmp_echo_reply local reply filter was attached without icmp_echo_reply=reply configured")
	}
}
