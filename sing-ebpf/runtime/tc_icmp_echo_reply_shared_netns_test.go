...
// frame: enough to check every property icmp_echo_reply promises (address,
...
func TestICMPEchoSharedReplyAnswersARealClientPing(t *testing.T) {
	...
	if attachment.sharedICMPFilter == nil {
		t.Fatal("the icmp_echo_reply shared filter was not attached alongside the ordinary one")
	}
