...
// newline compares equal to one sing-ebpf rewrote.
...
// changed while sing-ebpf was running. Restoring it would overwrite a setting
...
// knob changes underneath sing-ebpf. The original has to stay at the value the
// interface had before sing-ebpf touched it, but applied has to follow the most
// recent write or the restore guard mistakes sing-ebpf's own value for someone
...
func TestAppendTCSysctlStatesTracksLatestApplied(t *testing.T) {
	...
	// wlan0 must come back to the value it had before sing-ebpf ever pinned it.
	assertSysctl(t, "wlan0", "0")
}
