...
// (see assertICMPEchoReply's checkIPChecksum parameter).
...
// readICMPEchoReplyFrame reads frames off peerSocket, using parse to
...
// the identical loop TestICMPEchoSharedReplyAnswersARealClientPing and
// TestICMPEchoSharedRewriteAnswersARealClientPing already used.
func readICMPEchoReplyFrame(
	t *testing.T,
	peerSocket int,
...
// assertICMPEchoReply checks every property icmp_echo_reply's shared reply
...
func assertICMPEchoReply(
	...
	payload []byte,
	forceInterceptTarget, clientIP string,
	...
	if !reply.srcIP.Equal(net.ParseIP(forceInterceptTarget)) {
		t.Fatalf("reply source = %v, want it to still carry the ForceIntercept address %s", reply.srcIP, forceInterceptTarget)
	}
...
// TestICMPEchoLocalReplyAnswersARealIPv6PingViaClsact is
...
func TestICMPEchoLocalReplyAnswersARealIPv6PingViaClsact(t *testing.T) {
	...
	if attachment.localICMPFilter == nil {
		t.Fatal("the icmp_echo_reply clsact filter was not attached alongside the ordinary one")
	}

	before := icmpEchoReplyCount(t, backend)
	if err := pingICMPEchoReplyTargetV6(t, forceInterceptTarget, 5*time.Second); err != nil {
