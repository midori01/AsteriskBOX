...
// attachment helper in this package (attachICMPEchoReplyOrSkip,
...
func TestICMPEchoLocalReplyAnswersARealPingViaTCX(t *testing.T) {
	runForceInterceptLocalTCXReplyCase(t, false, "sbicmpx0", "sbicmpx1")
}

// TestICMPEchoReplyHealthCheckDetectsAndRepairsAMissingTCXLink is the TCX
// mirror of TestICMPEchoReplyHealthCheckDetectsAndRepairsAMissingClsactFilter:
...
func TestICMPEchoReplyHealthCheckDetectsAndRepairsAMissingTCXLink(t *testing.T) {
	enterTestNetworkNamespace(t)
	backend := newRealICMPEchoReplyBackend(t)
	t.Cleanup(func() { _ = backend.Close() })

	const forceInterceptTarget = "198.18.0.1"
	self := setupICMPEchoReplyPingVeth(t, "sbicmpy0", "sbicmpy1", forceInterceptTarget)

	const priority = 1
	attachment := attachICMPEchoReplyOrSkip(t, backend, "sbicmpy0", self.Attrs().Index, tcInterfaceRole{local: true}, priority)
	...
	// Simulate the icmp_echo_reply TCX link vanishing on its own, exactly as the
	...
	if err := detachable.Detach(); err != nil {
		t.Fatalf("detach the icmp_echo_reply TCX link to simulate drift: %v", err)
	}
	...
	if attached {
		t.Fatal("filtersAttached reported this attachment healthy with the icmp_echo_reply TCX link missing")
	}

	if err = pingICMPEchoReplyTarget(t, forceInterceptTarget, 2*time.Second); err == nil {
		t.Fatal("ping still answered after the icmp_echo_reply TCX link was closed — the drift is not real")
	}
	...
	if err = dataPlane.reconcile("sbicmpy0", nil, nil); err != nil {
		t.Fatalf("reconcile did not repair the missing icmp_echo_reply TCX link: %v", err)
	}
	...
	if repaired.localICMPLink == nil {
		t.Fatal("repair did not restore the icmp_echo_reply TCX link")
	}
...
// setupICMPEchoReplyPingVethIPv6 is setupICMPEchoReplyPingVeth's IPv6 counterpart:
// a veth pair, a global IPv6 address, a route to the ForceIntercept v6 range through
// it, a static neighbor entry for the ForceIntercept target (skipping the same
...
func setupICMPEchoReplyPingVethIPv6(t *testing.T, selfName, peerName, forceInterceptTarget string) netlink.Link {
	t.Helper()
	...
	if err = netlink.RouteAdd(forceInterceptRoute); err != nil {
		t.Fatalf("route the ForceIntercept v6 prefix through the veth: %v", err)
	}
	...
	if err = netlink.NeighAdd(neighbor); err != nil {
		t.Fatalf("add a static neighbor entry for the ForceIntercept v6 target: %v", err)
	}
	return self
}

// pingICMPEchoReplyTargetV6 is pingICMPEchoReplyTarget for ICMPv6: same real
// round trip, same amplification/address/checksum assertions, over
// golang.org/x/net/icmp and ipv6 instead of ipv4.
func pingICMPEchoReplyTargetV6(t *testing.T, forceInterceptTarget string, deadline time.Duration) error {
	t.Helper()
	...
	sequence := 9
	payload := []byte("force_intercept-icmp-v6-reply-test-payload")
	...
	// TestICMPEchoReplyPassThroughIntegration's real BPF_PROG_TEST_RUN cases in
	// the root sing-ebpf package, not re-derived here from a raw socket that cannot see it.
	return nil
}

// TestICMPEchoLocalReplyAnswersARealIPv6PingViaTCX covers the IPv6 local
// reply through a real TCX attachment.
func TestICMPEchoLocalReplyAnswersARealIPv6PingViaTCX(t *testing.T) {
	runForceInterceptLocalTCXReplyCase(t, true, "sbicmpz0", "sbicmpz1")
}
