//go:build with_ebpf && linux && ebpf_integration

package runtime

import (
	"net"
	"net/netip"
	"testing"
	"time"

	commonEBPF "github.com/CHIZI-0618/sing-ebpf"
	"github.com/sagernet/netlink"

	"golang.org/x/sys/unix"
)

func newRealICMPEchoReplyBackend(t *testing.T) *commonEBPF.TCBackend {
	return newRealICMPEchoReplyBackendFor(t, false)
}

func newRealICMPEchoReplyBackendWithIPv6(t *testing.T) *commonEBPF.TCBackend {
	return newRealICMPEchoReplyBackendFor(t, true)
}

func newRealICMPEchoReplyBackendFor(t *testing.T, enableIPv6 bool) *commonEBPF.TCBackend {
	t.Helper()
	policyConfig := commonEBPF.PolicyConfig{
		EnableTCP:          true,
		ForceInterceptIPv4: netip.MustParsePrefix("198.18.0.0/15"),
	}
	if enableIPv6 {
		policyConfig.ForceInterceptIPv6 = netip.MustParsePrefix("fc00::/18")
	}
	policy, err := commonEBPF.CompilePolicy(policyConfig)
	if err != nil {
		t.Fatalf("compile policy: %v", err)
	}
	backend, err := commonEBPF.PrepareTC(commonEBPF.TCConfig{
		ListenerPort:     23456,
		EnableLocal:      true,
		EnableShared:     true,
		EnableIPv4:       true,
		EnableLocalIPv6:  enableIPv6,
		EnableSharedIPv6: enableIPv6,
		EnableTCP:        true,
		Policy:           policy,
		ICMPEchoReply:    true,
	})
	if err != nil {
		t.Skipf("cannot prepare a real TC eBPF backend in this environment: %v", err)
	}
	return backend
}

func newRealICMPEchoSharedPacketRewriteBackend(t *testing.T) *commonEBPF.SharedPacketRewriteBackend {
	return newRealICMPEchoSharedPacketRewriteBackendFor(t, false)
}

func newRealICMPEchoSharedPacketRewriteBackendFor(t *testing.T, enableIPv6 bool) *commonEBPF.SharedPacketRewriteBackend {
	t.Helper()
	policyConfig := commonEBPF.PolicyConfig{
		EnableTCP:          true,
		ForceInterceptIPv4: netip.MustParsePrefix("198.18.0.0/15"),
	}
	config := commonEBPF.SharedPacketRewriteConfig{
		ListenerPort:  23458,
		EnableTCP:     true,
		RedirectIPv4:  netip.MustParsePrefix("127.128.0.0/9"),
		MapCapacity:   commonEBPF.DefaultSharedPacketRewriteMapCapacity(),
		UDPTimeout:    5 * time.Minute,
		ICMPEchoReply: true,
	}
	if enableIPv6 {
		policyConfig.ForceInterceptIPv6 = netip.MustParsePrefix("fc00::/18")
		config.RedirectIPv6 = netip.MustParsePrefix("fd53:696e:672d:626f::/64")
	}
	policy, err := commonEBPF.CompilePolicy(policyConfig)
	if err != nil {
		t.Fatalf("compile policy: %v", err)
	}
	config.Policy = policy
	backend, err := commonEBPF.PrepareSharedPacketRewrite(nil, config)
	if err != nil {
		t.Skipf("cannot prepare a real shared-network eBPF backend in this environment: %v", err)
	}
	return backend
}

func attachICMPEchoReplyOrSkip(
	t *testing.T,
	backend *commonEBPF.TCBackend,
	interfaceName string,
	index int,
	role tcInterfaceRole,
	priority uint16,
) *tcInterfaceAttachment {
	t.Helper()
	lock, err := acquireTCInterfaceLock(interfaceName, index)
	if err != nil {
		t.Fatalf("acquire the interface lock: %v", err)
	}
	attachment, err := attachTCInterfaceWithLock(
		netlink.LinkByName,
		backend,
		interfaceName,
		tcAttachmentState{index: index, framing: commonEBPF.TCLinkFramingEthernet, role: role},
		false,
		priority,
		lock,
		true,
	)
	if err != nil {
		t.Fatalf("attach the interface: %v", err)
	}
	if priority == defaultTCPriority && attachment.attachmentType != "tcx" {
		_ = attachment.Close()
		requireOrSkipTCX(t, attachment.attachmentType)
	}
	return attachment
}

func attachSharedRewriteOrSkip(
	t *testing.T,
	device netlink.Link,
	backend *commonEBPF.SharedPacketRewriteBackend,
	priority uint16,
) *sharedRewriteAttachment {
	t.Helper()
	attachment, err := attachSharedRewriteInterface(device, backend, priority)
	if err != nil {
		t.Fatalf("attach the shared packet-rewrite interface: %v", err)
	}
	if priority == defaultTCPriority && attachment.attachmentType != "tcx" {
		_ = attachment.Close()
		requireOrSkipTCX(t, attachment.attachmentType)
	}
	return attachment
}

type icmpEchoReplyCounter interface {
	ICMPEchoReplyCount() (uint64, error)
}

func icmpEchoReplyCount(t *testing.T, counter icmpEchoReplyCounter) uint64 {
	t.Helper()
	count, err := counter.ICMPEchoReplyCount()
	if err != nil {
		t.Fatalf("read ICMPEchoReplyCount: %v", err)
	}
	return count
}

func requireOneICMPEchoReply(t *testing.T, counter icmpEchoReplyCounter, before uint64) {
	t.Helper()
	after := icmpEchoReplyCount(t, counter)
	if after != before+1 {
		t.Fatalf("ICMPEchoReplyCount = %d, want %d after one successfully answered ping", after, before+1)
	}
}

type forceInterceptSharedDataPlane uint8

const (
	forceInterceptSharedSocketAssign forceInterceptSharedDataPlane = iota
	forceInterceptSharedPacketRewrite
)

type forceInterceptSharedReplyCase struct {
	dataPlane  forceInterceptSharedDataPlane
	ipv6       bool
	priority   uint16
	selfName   string
	peerName   string
	client     string
	identifier uint16
	sequence   uint16
}

func runForceInterceptSharedReplyCase(t *testing.T, testCase forceInterceptSharedReplyCase) {
	t.Helper()
	enterTestNetworkNamespace(t)
	self, peer := createTestVethPair(t, testCase.selfName, testCase.peerName)

	var counter icmpEchoReplyCounter
	switch testCase.dataPlane {
	case forceInterceptSharedSocketAssign:
		backend := newRealICMPEchoReplyBackendFor(t, testCase.ipv6)
		t.Cleanup(func() { _ = backend.Close() })
		counter = backend
		attachment := attachICMPEchoReplyOrSkip(
			t, backend, testCase.selfName, self.Attrs().Index, tcInterfaceRole{shared: true}, testCase.priority,
		)
		t.Cleanup(func() { _ = attachment.Close() })
		if testCase.priority == defaultTCPriority && attachment.sharedICMPLink == nil {
			t.Fatal("the ForceIntercept ICMP TCX link was not attached")
		}
		if testCase.priority != defaultTCPriority && attachment.sharedICMPFilter == nil {
			t.Fatal("the ForceIntercept ICMP clsact filter was not attached")
		}
	case forceInterceptSharedPacketRewrite:
		backend := newRealICMPEchoSharedPacketRewriteBackendFor(t, testCase.ipv6)
		t.Cleanup(func() { _ = backend.Close() })
		counter = backend
		attachment := attachSharedRewriteOrSkip(t, self, backend, testCase.priority)
		t.Cleanup(func() { _ = attachment.Close() })
		if testCase.priority == defaultTCPriority && attachment.icmpLink == nil {
			t.Fatal("the ForceIntercept ICMP TCX link was not attached")
		}
		if testCase.priority != defaultTCPriority && attachment.icmpFilter == nil {
			t.Fatal("the ForceIntercept ICMP clsact filter was not attached")
		}
	default:
		t.Fatalf("unknown shared data plane %d", testCase.dataPlane)
	}

	before := icmpEchoReplyCount(t, counter)
	forceInterceptTarget := "198.18.0.1"
	etherType := uint16(unix.ETH_P_IP)
	requestType, replyType := uint8(8), uint8(0)
	parse := parseEthernetIPv4ICMP
	build := buildEthernetIPv4EchoRequest
	checkIPChecksum := true
	if testCase.ipv6 {
		forceInterceptTarget = "fc00::1"
		etherType = unix.ETH_P_IPV6
		requestType, replyType = 128, 129
		parse = parseEthernetIPv6ICMP
		build = buildEthernetIPv6EchoRequest
		checkIPChecksum = false
	}
	payload := []byte(t.Name())
	request := build(
		self.Attrs().HardwareAddr, peer.Attrs().HardwareAddr,
		net.ParseIP(testCase.client), net.ParseIP(forceInterceptTarget),
		testCase.identifier, testCase.sequence, payload,
	)
	socket := openRawLinkLayerSocket(t, peer.Attrs().Index, etherType, 5*time.Second)
	if _, err := unix.Write(socket, request); err != nil {
		t.Fatalf("transmit the echo request: %v", err)
	}
	reply, replyLength := readICMPEchoReplyFrame(t, socket, requestType, parse)
	assertICMPEchoReply(
		t, reply, replyLength, len(request), replyType,
		testCase.identifier, testCase.sequence, payload, forceInterceptTarget, testCase.client,
		peer.Attrs().HardwareAddr, self.Attrs().HardwareAddr, checkIPChecksum,
	)
	requireOneICMPEchoReply(t, counter, before)
}

func runForceInterceptLocalTCXReplyCase(t *testing.T, ipv6 bool, selfName, peerName string) {
	t.Helper()
	enterTestNetworkNamespace(t)
	backend := newRealICMPEchoReplyBackendFor(t, ipv6)
	t.Cleanup(func() { _ = backend.Close() })
	var self netlink.Link
	var ping func(*testing.T, string, time.Duration) error
	forceInterceptTarget := "198.18.0.1"
	if ipv6 {
		forceInterceptTarget = "fc00::1"
		self = setupICMPEchoReplyPingVethIPv6(t, selfName, peerName, forceInterceptTarget)
		ping = pingICMPEchoReplyTargetV6
	} else {
		self = setupICMPEchoReplyPingVeth(t, selfName, peerName, forceInterceptTarget)
		ping = pingICMPEchoReplyTarget
	}
	attachment := attachICMPEchoReplyOrSkip(
		t, backend, selfName, self.Attrs().Index, tcInterfaceRole{local: true}, defaultTCPriority,
	)
	t.Cleanup(func() { _ = attachment.Close() })
	if attachment.localICMPLink == nil {
		t.Fatal("the ForceIntercept ICMP TCX link was not attached")
	}
	if err := ping(t, forceInterceptTarget, 5*time.Second); err != nil {
		t.Fatalf("read a reply: %v", err)
	}
}
