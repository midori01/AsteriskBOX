...
// filtersAttached — see TestICMPEchoReplyHealthCheckDetectsAndRepairsAMissingClsactFilter
// — already confirmed before deciding a repair is even needed), and only
// the icmp_echo_reply filter is missing. A transient failure repairing just the
...
func TestUpdateTCInterfaceAttachmentRepairRetainsAHealthyFilterOnICMPFailure(t *testing.T) {
	...
	ops := tcInterfaceAttachmentOps{
		...
		attachFilter: func(_ netlink.Link, _ uint32, _ int, name string, _ uint16, _ uint16) (*netlink.BpfFilter, error) {
			...
			if icmpAttachAttempts == 1 {
				return nil, errors.New("injected icmp_echo_reply attach failure")
			}
			return &netlink.BpfFilter{}, nil
		},
	...
	if err := update(); err == nil {
		t.Fatal("expected the injected icmp_echo_reply attach failure")
	}
	...
	if attachment.localICMPFilter == nil {
		t.Fatal("the recovered repair did not attach the icmp_echo_reply filter")
	}
	if normalAttachAttempts != 0 {
		t.Fatalf(
	...
	if icmpAttachAttempts != 2 {
		t.Fatalf("icmp_echo_reply attach attempts = %d, want exactly 2 (one failure, one success)", icmpAttachAttempts)
	}
}
