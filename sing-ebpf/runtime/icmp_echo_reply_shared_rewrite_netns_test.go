...
// Ethernet+IPv4+UDP frame, for the coexistence proof: icmp_echo_reply must leave
...
func buildEthernetIPv4UDPPacket(dstMAC, srcMAC net.HardwareAddr, srcIP, dstIP net.IP, srcPort, dstPort uint16, payload []byte) []byte {
	...
	// for whether icmp_echo_reply mistakenly answers it, not consumed by any
	// real UDP stack.

	return frame
}

func TestSharedRewriteClsactReplacementPreservesICMPEchoReplyFilter(t *testing.T) {
	enterTestNetworkNamespace(t)
	backend := newRealICMPEchoSharedPacketRewriteBackend(t)
	...
	if !healthy {
		t.Fatal("retiring the old clsact attachment removed a staged ForceIntercept ICMP filter")
	}
}

// TestICMPEchoSharedRewriteAnswersARealClientPing verifies the
// packet-rewrite attachment with a real link-layer request and reply.
func TestICMPEchoSharedRewriteAnswersARealClientPing(t *testing.T) {
	enterTestNetworkNamespace(t)
	backend := newRealICMPEchoSharedPacketRewriteBackend(t)
	t.Cleanup(func() { _ = backend.Close() })
	...
	if attachment.icmpFilter == nil {
		t.Fatal("the icmp_echo_reply shared filter was not attached alongside packet_rewrite's own ingress/egress filters")
	}
	if attachment.ingressFilter == nil || attachment.egressFilter == nil {
		t.Fatal("packet_rewrite's own ingress/egress filters are missing -- icmp_echo_reply must not replace them")
	}
	...
	const sequence = 7
	payload := []byte("force_intercept-icmp-shared-rewrite-test-payload")

	requestFrame := buildEthernetIPv4EchoRequest(
		self.Attrs().HardwareAddr, peer.Attrs().HardwareAddr,
		net.ParseIP(clientIP), net.ParseIP(forceInterceptTarget),
	...
	reply, replyLength := readICMPEchoReplyFrame(t, peerSocket, 8, parseEthernetIPv4ICMP)
	assertICMPEchoReply(
		t, reply, replyLength, len(requestFrame), 0,
		identifier, sequence, payload, forceInterceptTarget, clientIP,
		peer.Attrs().HardwareAddr, self.Attrs().HardwareAddr, true,
	)
	requireOneICMPEchoReply(t, backend, repliesBefore)
}

// TestICMPEchoSharedRewriteIgnoresNonICMPToForceInterceptTarget verifies that the
// responder does not shadow packet-rewrite TCP/UDP handling.
func TestICMPEchoSharedRewriteIgnoresNonICMPToForceInterceptTarget(t *testing.T) {
	enterTestNetworkNamespace(t)
	backend := newRealICMPEchoSharedPacketRewriteBackend(t)
	t.Cleanup(func() { _ = backend.Close() })
	...
	for {
		...
		if readErr != nil {
			...
			// nothing at all): icmp_echo_reply correctly left this alone.
			return
		}
		frame := buffer[:n]
		const ethernetLength = 14
		...
		if reply.icmpType == 0 {
			t.Fatal("icmp_echo_reply answered a UDP packet with an ICMP Echo Reply -- protocol discrimination failed")
		}
	}
}
