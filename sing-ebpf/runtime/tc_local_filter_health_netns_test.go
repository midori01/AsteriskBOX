...
// ordinary local filter independently of the optional ForceIntercept ICMP filter.
func TestTCLocalFilterHealthCheckDetectsAndRepairsExternalDeletion(t *testing.T) {
	...
	if attachment.localICMPFilter != nil {
		t.Fatal("icmp_echo_reply is disabled on this backend; its filter must not have been attached")
	}
