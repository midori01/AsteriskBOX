...
// tcxAttachedLink is the subset of link.Link the icmp_echo_reply TCX link fields
...
type tcInterfaceAttachment struct {
	...
	// icmp_echo_reply reply filters, attached alongside localFilter/sharedFilter
	// under the same role and only when backend.ICMPEchoReplyEnabled(); nil
...
func (d *tcDataPlane) attachmentDiagnostics() []commonEBPF.AttachmentInfo {
	...
	for _, attachment := range d.attachments {
		icmpEchoReply := attachment.localICMPFilter != nil || attachment.sharedICMPFilter != nil ||
...
func attachTCXInterface(linkDevice netlink.Link, backend *commonEBPF.TCBackend, attachment *tcInterfaceAttachment) (bool, error) {
	...
	icmpEchoReplyEnabled := backend.ICMPEchoReplyEnabled()
	pairs := []struct {
		enabled     bool
		role        string
		...
		{attachment.role.local, "local", CiliumEBPF.AttachTCXEgress,
			rawTCBackend(backend).LocalEgressProgram(attachment.framing), rawTCBackend(backend).ICMPEchoLocalReplyProgram(attachment.framing),
			&attachment.localLink, &attachment.localICMPLink},
		{attachment.role.shared, "shared", CiliumEBPF.AttachTCXIngress,
			rawTCBackend(backend).SharedIngressProgram(attachment.framing), rawTCBackend(backend).ICMPEchoSharedReplyProgram(attachment.framing),
...
func updateTCInterfaceAttachmentWithOps(
	...
	if !role.shared {
		if err = detachTCFilterOwnedWith(&attachment.sharedICMPFilter, ops.detachFilter); err != nil {
			return rollbackAdded(E.Cause(err, "detach icmp_echo_reply shared reply filter from interface ", attachment.interfaceName))
		}
	...
	if !role.local {
		if err = detachTCFilterOwnedWith(&attachment.localICMPFilter, ops.detachFilter); err != nil {
			return rollbackAdded(E.Cause(err, "detach icmp_echo_reply local reply filter from interface ", attachment.interfaceName))
		}
...
// folds icmp_echo_reply link health into an unchanged role's own "is this role
...
func updateTCXInterfaceAttachment(
	...
	attach := func(local bool) error {
		...
		if backend.ICMPEchoReplyEnabled() {
			...
			icmpProgram := rawTCBackend(backend).ICMPEchoSharedReplyProgram(attachment.framing)
			if local {
				icmpProgram = rawTCBackend(backend).ICMPEchoLocalReplyProgram(attachment.framing)
			}
			if icmpProgram == nil {
				startErr := E.New("icmp_echo_reply shared reply program is unavailable")
				if local {
					startErr = E.New("icmp_echo_reply local reply program is unavailable")
				}
				if added {
	...
	if err := transitionTCXInterfaceRole(
		attachment.role,
		role,
		attachment.localLink != nil && (!backend.ICMPEchoReplyEnabled() || attachment.localICMPLink != nil),
		attachment.sharedLink != nil && (!backend.ICMPEchoReplyEnabled() || attachment.sharedICMPLink != nil),
		attach,
		detach,
	); err != nil {
...
// link is missing anyway (the caller folds icmp_echo_reply link health into
