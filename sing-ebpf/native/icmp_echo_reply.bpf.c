...
// ForceIntercept prefixes with a locally synthesized Echo Reply, so a client's ping
// tool sees the ForceIntercept address as reachable without the request ever leaving
...
#define SB_ICMP_ECHO_REPLY_FLAG_ENABLED (1U << 0)
#define SB_ICMP_ECHO_REPLY_FLAG_IPV4 (1U << 1)
#define SB_ICMP_ECHO_REPLY_FLAG_LOCAL_IPV6 (1U << 2)
#define SB_ICMP_ECHO_REPLY_FLAG_SHARED_IPV6 (1U << 3)
#define SB_ICMP_ECHO_REPLY_FLAG_FORCE_INTERCEPT_IPV4 (1U << 4)
#define SB_ICMP_ECHO_REPLY_FLAG_FORCE_INTERCEPT_IPV6 (1U << 5)

// Independent of sb_tc_control by design: this object only ever needs the
// address-family toggles and the ForceIntercept prefixes themselves, not the rest of
// the TC data plane's policy surface.
struct sb_icmp_echo_reply_control {
    __u32 flags;
...
_Static_assert(sizeof(struct sb_icmp_echo_reply_control) == 44, "sb_icmp_echo_reply_control ABI size");
_Static_assert(__builtin_offsetof(struct sb_icmp_echo_reply_control, force_intercept_ipv6_mask) == 28,
    "sb_icmp_echo_reply_control force_intercept_ipv6_mask ABI offset");
...
MAP(icmp_echo_reply_control, __u32, struct sb_icmp_echo_reply_control, BPF_MAP_TYPE_ARRAY, 1U);

// icmp_echo_reply_stats counts three outcomes for this object's own decisions,
...
// wrong type/code, an extension header, or a destination outside the ForceIntercept
...
// this program is attached to, not a fact about icmp_echo_reply's own behavior.
#define SB_ICMP_ECHO_REPLY_STAT_REPLY 0U
#define SB_ICMP_ECHO_REPLY_STAT_PASS_THROUGH 1U
#define SB_ICMP_ECHO_REPLY_STAT_REWRITE_FAILURE 2U
#define SB_ICMP_ECHO_REPLY_STAT_COUNT 3U

MAP(icmp_echo_reply_stats, __u32, __u64, BPF_MAP_TYPE_PERCPU_ARRAY, SB_ICMP_ECHO_REPLY_STAT_COUNT);
...
INLINE const struct sb_icmp_echo_reply_control *load_control(void) {
    __u32 zero = 0U;
    return map_lookup(&icmp_echo_reply_control, &zero);
}
...
// not a fact about icmp_echo_reply's own behavior. Once a packet is known to be
...
// what SB_ICMP_ECHO_REPLY_STAT_PASS_THROUGH exists to count.
INLINE bool find_ipv4_echo_request(void *data, void *data_end, __u32 packet_length, __u32 l3_offset,
    const struct sb_icmp_echo_reply_control *control, struct ipv4_header **ip, struct icmp_echo_header **icmp) {
    ...
    if (header->version != 4U || header->ihl != 5U) {
        record_icmp_echo_reply_stat(SB_ICMP_ECHO_REPLY_STAT_PASS_THROUGH);
        return false;
    }
    ...
    if (!sb_ebpf_force_intercept_ipv4(
            destination, control->flags, SB_ICMP_ECHO_REPLY_FLAG_FORCE_INTERCEPT_IPV4,
            control->force_intercept_ipv4_prefix, control->force_intercept_ipv4_mask)) {
        record_icmp_echo_reply_stat(SB_ICMP_ECHO_REPLY_STAT_PASS_THROUGH);
        return false;
    }
...
INLINE bool find_ipv6_echo_request(void *data, void *data_end, __u32 packet_length, __u32 l3_offset,
    const struct sb_icmp_echo_reply_control *control, struct ipv6_header **ip, struct icmp_echo_header **icmp) {
    ...
    if (!sb_ebpf_force_intercept_ipv6(
            header->destination, control->flags, SB_ICMP_ECHO_REPLY_FLAG_FORCE_INTERCEPT_IPV6,
            control->force_intercept_ipv6_prefix, control->force_intercept_ipv6_mask)) {
        record_icmp_echo_reply_stat(SB_ICMP_ECHO_REPLY_STAT_PASS_THROUGH);
        return false;
    }
...
INLINE int reply_ipv4_echo(struct __sk_buff *skb, __u32 l3_offset, __be32 old_source, __be32 old_destination, __u8 old_type, __u8 old_code) {
    ...
    if (l3_csum_replace(skb, checksum_offset, old_source, new_source, 4U) != 0 ||
        ...
        record_icmp_echo_reply_stat(SB_ICMP_ECHO_REPLY_STAT_REWRITE_FAILURE);
        return TC_ACT_SHOT;
    }
    record_icmp_echo_reply_stat(SB_ICMP_ECHO_REPLY_STAT_REPLY);
    return TC_ACT_OK;
}
...
INLINE int reply_ipv6_echo(struct __sk_buff *skb, __u32 l3_offset, const __u8 old_addresses[32], __u8 old_type, __u8 old_code) {
    ...
    if (address_diff < 0) { record_icmp_echo_reply_stat(SB_ICMP_ECHO_REPLY_STAT_REWRITE_FAILURE); return TC_ACT_SHOT; }
...
// icmp_echo_reply off leaves that filter and everything it does untouched.
//
...
INLINE int local_reply(struct __sk_buff *skb, bool ethernet) {
    ...
    if (ether_type == ETH_P_IP_VALUE && (control->flags & SB_ICMP_ECHO_REPLY_FLAG_IPV4) != 0U) {
        ...
        if (ethernet && !swap_ethernet_addresses(skb)) { record_icmp_echo_reply_stat(SB_ICMP_ECHO_REPLY_STAT_REWRITE_FAILURE); return TC_ACT_SHOT; }
    ...
    if (ether_type == ETH_P_IPV6_VALUE && (control->flags & SB_ICMP_ECHO_REPLY_FLAG_LOCAL_IPV6) != 0U) {
...
SEC("classifier/icmp_echo_reply_local_reply_ethernet")
int sing_ebpf_icmp_echo_reply_local_reply_ethernet(struct __sk_buff *skb) {
    return local_reply(skb, true);
}

SEC("classifier/icmp_echo_reply_local_reply_raw_ip")
int sing_ebpf_icmp_echo_reply_local_reply_raw_ip(struct __sk_buff *skb) {
...
INLINE int shared_reply(struct __sk_buff *skb, bool ethernet) {
    ...
    if (ether_type == ETH_P_IPV6_VALUE && (control->flags & SB_ICMP_ECHO_REPLY_FLAG_SHARED_IPV6) != 0U) {
        struct ipv6_header *ip;
        struct icmp_echo_header *icmp;
        if (!find_ipv6_echo_request(data, data_end, skb->len, l3_offset, control, &ip, &icmp)) return TC_ACT_UNSPEC;
        __u8 old_addresses[32];
...
SEC("classifier/icmp_echo_reply_shared_reply_ethernet")
int sing_ebpf_icmp_echo_reply_shared_reply_ethernet(struct __sk_buff *skb) {
    return shared_reply(skb, true);
}

SEC("classifier/icmp_echo_reply_shared_reply_raw_ip")
int sing_ebpf_icmp_echo_reply_shared_reply_raw_ip(struct __sk_buff *skb) {
