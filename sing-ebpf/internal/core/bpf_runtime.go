...
func eBPFOperationError(operation string, err error) error {
	...
	if errors.As(err, &errno) {
		switch errno {
		case unix.EBUSY:
			return E.Cause(errno, "another eBPF data plane is already active on this attach point: ", operation)
...
func validateCgroup2Mount(scope string, cgroupPath string) error {
	...
	if fileSystem.Type != unix.CGROUP2_SUPER_MAGIC {
		return E.New("eBPF data plane is not supported: ", cgroupPath, " is not a cgroup2 mount")
	}
	return nil
}
