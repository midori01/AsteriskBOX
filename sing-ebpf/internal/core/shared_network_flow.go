...
func (b *SharedPacketRewriteBackend) LookupFlow(
	protocol uint8,
	client netip.AddrPort,
	tokenDestination netip.AddrPort,
) (OriginalDestination, *SharedPacketRewriteFlowHandle, error) {
	if b == nil {
	...
	flow := makeSharedPacketRewriteFlowHandle(key, value)
...
func (b *SharedPacketRewriteBackend) ReserveUDPReplyFlow(
	base *SharedPacketRewriteFlowHandle,
	destination netip.AddrPort,
	sourceMAC net.HardwareAddr,
) (netip.Addr, *SharedPacketRewriteFlowHandle, error) {
	if b == nil {
	...
	for attempt := 0; attempt < userspaceReplyTokenAttempts; {
		...
		flow := makeSharedPacketRewriteFlowHandleFromOriginal(
			originalKey,
			tokenAddress,
...
func (b *SharedPacketRewriteBackend) lookupUDPReplyFlowLocked(
	originalKey sharedNetworkOriginalKey,
) (netip.Addr, *SharedPacketRewriteFlowHandle, bool, error) {
...
func (b *SharedPacketRewriteBackend) udpReplyTokenPrefix(family uint8) (netip.Prefix, error) {
	switch family {
	case addressFamilyIPv4:
		if b.control.Flags&sharedPacketRewriteFlagIPv4 == 0 {
			break
		}
	...
	case addressFamilyIPv6:
		if b.control.Flags&sharedPacketRewriteFlagIPv6 == 0 {
			break
		}
		return netip.PrefixFrom(
...
func (b *SharedPacketRewriteBackend) deferTCPFlowReleaseLocked(flow SharedPacketRewriteFlowHandle, now time.Time) {
	if b.flowReleases == nil {
		b.flowReleases = make(map[SharedPacketRewriteFlowHandle]time.Time)
	}
...
func (b *SharedPacketRewriteBackend) signalFlowWake() {
	select {
...
func (b *SharedPacketRewriteBackend) validateFlowGenerationLocked(flow SharedPacketRewriteFlowHandle) error {
...
func (b *SharedPacketRewriteBackend) deleteFlowGenerationLocked(flow SharedPacketRewriteFlowHandle) (bool, error) {
	var current sharedNetworkTokenValue
...
func (b *SharedPacketRewriteBackend) deleteTokenGenerationLocked(flow SharedPacketRewriteFlowHandle) error {
...
func (b *SharedPacketRewriteBackend) retainFlowLocked(flow SharedPacketRewriteFlowHandle) {
	delete(b.flowReleases, flow)
	if b.flowReferences == nil {
		b.flowReferences = make(map[SharedPacketRewriteFlowHandle]uint32)
	}
...
func (b *SharedPacketRewriteBackend) releaseFlowReferenceLocked(flow SharedPacketRewriteFlowHandle) bool {
...
func (b *SharedPacketRewriteBackend) SweepOrphanedFlows(
	maxIdle time.Duration,
	fallbackBudget uint32,
) (SharedPacketRewriteSweepResult, error) {
	if b == nil {
		return SharedPacketRewriteSweepResult{}, errBackendClosed
	}
	if maxIdle <= 0 || fallbackBudget == 0 {
		return SharedPacketRewriteSweepResult{}, unix.EINVAL
	}
	...
	var now unix.Timespec
	if err := unix.ClockGettime(unix.CLOCK_MONOTONIC, &now); err != nil {
		return SharedPacketRewriteSweepResult{}, err
	}
	nowNS := uint64(now.Sec)*uint64(time.Second) + uint64(now.Nsec)
	maxIdleNS := uint64(maxIdle)
	if nowNS <= maxIdleNS {
		return SharedPacketRewriteSweepResult{
	...
	result := SharedPacketRewriteSweepResult{
...
func (b *SharedPacketRewriteBackend) removeOrphanedFlowCandidate(
	mapFD int,
