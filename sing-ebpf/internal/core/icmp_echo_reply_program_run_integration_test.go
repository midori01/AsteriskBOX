...
// TestICMPEchoReplyPassThroughIntegration executes the native classifier with
...
func TestICMPEchoReplyPassThroughIntegration(t *testing.T) {
	...
	testCases := []struct {
		...
		{"non-ForceIntercept destination", testIPv4EchoPacket(netip.MustParseAddr("192.0.2.10"), netip.MustParseAddr("203.0.113.1"))},
	...
	controlMap := backend.icmpEchoReply.runtime.maps["icmp_echo_reply_control"]
	zero := uint32(0)
	disabled := icmpEchoReplyControl{}
	if err = controlMap.Update(&zero, &disabled, 0); err != nil {
		t.Fatalf("disable icmp_echo_reply control: %v", err)
	}
