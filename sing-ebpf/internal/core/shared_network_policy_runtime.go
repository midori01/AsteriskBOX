...
func (b *SharedPacketRewriteBackend) initializeSourceCIDRPolicy(include, exclude dualStackCIDRPrefixes) error {
	var err error
...
func (b *SharedPacketRewriteBackend) initializeSourceMACPolicy(include, exclude []MACAddress) error {
...
func (b *SharedPacketRewriteBackend) updatePolicyFlagsLocked() error {
	...
	bypassFlowCacheFlag := b.control.Flags & sharedPacketRewriteFlagBypassFlowCache
	...
	if sharedNetworkBypassFlowCacheRequired(policyFlags) {
		policyFlags |= sharedPacketRewriteFlagBypassFlowCache
	}
