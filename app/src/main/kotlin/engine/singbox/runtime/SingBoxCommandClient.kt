...
import engine.singbox.NetworkQualityPhase
import engine.singbox.NetworkQualityProgress
import engine.singbox.NetworkQualityTestHandler
import engine.singbox.NetworkQualityTestSession
...
internal class SingBoxCommandClient(
     ...
     *
     * Forwards to `/daemon.StartedService/StartNetworkQualityTest` on the gRPC
     * control channel used by every other runtime command. Caller must `close()`
     * the returned [NetworkQualityTestSession] to cancel; the handler keeps
     * receiving callbacks until either `onResult` or `onError`.
     */
    fun startNetworkQualityTest(
    ...
    ): NetworkQualityTestSession {
        val writer = ProtoWriter()
        if (configURL.isNotEmpty()) writer.writeString(1, configURL)
        if (outboundTag.isNotEmpty()) writer.writeString(2, outboundTag)
        if (serial) writer.writeBool(3, serial)
        if (maxRuntimeSeconds != 0) writer.writeInt32(4, maxRuntimeSeconds)
        if (http3) writer.writeBool(5, http3)
        val requestPayload = writer.toByteArray()

        val job = CoroutineScope(Dispatchers.IO + SupervisorJob()).launch {
            var conn: HttpURLConnection? = null
            try {
                val url = URL("${target.control.baseUrl}/daemon.StartedService/StartNetworkQualityTest")
                ...
                if (responseCode !in 200..299) {
                    ...
                    handler.onError("HTTP $responseCode ${connection.responseMessage}: $err")
                    return@launch
                }
                ...
                connection.inputStream.use { input ->
                    while (isActive && !isClosed.get()) {
                        ...
                        if (frame.isTrailer) {
                            ...
                            val progress = decodeNetworkQualityProgress(frame.payload)
                            if (progress.error != null) {
                                handler.onError(progress.error)
                                return@launch
                            } else if (progress.finished || progress.phase == NetworkQualityPhase.Done) {
                                handler.onResult(progress)
                                return@launch
                            } else {
                ...
                if (e !is CancellationException) {
                    handler.onError(e.message ?: "Network quality test failed")
                }
            } finally {
                conn?.let {
        ...
        return object : NetworkQualityTestSession {
            override fun close() {
                job.cancel()
            }
        }
...
private fun decodeNetworkQualityProgress(bytes: ByteArray): NetworkQualityProgress {
    val reader = ProtoReader(bytes)
    var phase = 0
    var downloadCapacity = 0L
    var uploadCapacity = 0L
    var downloadRPM = 0
    var uploadRPM = 0
    var idleLatencyMs = 0
    var elapsedMs = 0L
    ...
    var downloadCapacityAccuracy = 0
    var uploadCapacityAccuracy = 0
    var downloadRPMAccuracy = 0
    var uploadRPMAccuracy = 0

    while (reader.hasNext) {
        val tag = reader.readTag() ?: break
        when (tag.first) {
            1 -> phase = reader.readVarint().toInt()
            2 -> downloadCapacity = reader.readVarint()
            3 -> uploadCapacity = reader.readVarint()
            4 -> downloadRPM = reader.readVarint().toInt()
            5 -> uploadRPM = reader.readVarint().toInt()
            6 -> idleLatencyMs = reader.readVarint().toInt()
            7 -> elapsedMs = reader.readVarint()
            8 -> isFinal = reader.readVarint() != 0L
            9 -> error = reader.readString().takeIf { it.isNotEmpty() }
            10 -> downloadCapacityAccuracy = reader.readVarint().toInt()
            11 -> uploadCapacityAccuracy = reader.readVarint().toInt()
            12 -> downloadRPMAccuracy = reader.readVarint().toInt()
            13 -> uploadRPMAccuracy = reader.readVarint().toInt()
            else -> reader.skipField(tag.second)
        }
    }
    return NetworkQualityProgress(
        ...
        error = error,
        finished = isFinal || error != null || phase == NetworkQualityPhase.Done.wire,
    )
}
